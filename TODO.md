...
- [ ] **Gör om Miljöer-sektionen på startsidan så att mängden piller försvinner.** Ta bort nuvarande pillerlösning och ta fram ett lugnare visuellt sätt att visa miljöerna utan att förlora tydlig navigering. Utvärdera i första hand diskreta textlänkar, en kompakt tvåkolumnslista eller tydliga miljökort med namn och enkel pil/indikator i stället för många fristående etiketter. Undvik att ersätta pillren med lika många visuellt tunga knappar. Lösningen ska fungera på desktop och mobil, behålla klickbarhet och internlänkning samt passa startsidans övriga formspråk. Gör gärna 2–3 visuella alternativ innan implementation och välj den variant som ger minst visuellt brus.
- [ ] **Bygg om filtreringen på referensöversikten till en kompakt webbshop-liknande filterlösning.** Nuvarande filter ligger permanent expanderade som många piller och tar för stor visuell plats. Ersätt dem med en samlad filterkontroll där användaren öppnar en panel/dropdown/drawer och väljer exempelvis miljö, tjänst, ort eller andra relevanta kategorier. Visa aktiva val kompakt och ge tydlig möjlighet att rensa filter. På desktop kan filtreringen vara en kompakt sidopanel eller dropdownrad; på mobil bör den öppnas som en tydlig `Filtrera`-knapp med panel/drawer i stället för att alla val visas samtidigt. Behåll snabb filtrering, tillgängliga checkboxar/radioval, tangentbordsstöd, tydligt antal träffar och korrekt URL-/state-hantering om det redan finns eller är motiverat. Undvik att bygga en tung e-handelslösning om referensmängden är liten; målet är framför allt mindre visuellt brus och enklare överblick.
