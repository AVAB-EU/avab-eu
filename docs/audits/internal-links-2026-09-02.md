...
### 12.5 Historiskt före/efter (Fas B, 2026-09-04)
...
5 P0-förekomster (3 poster) åtgärdades via verifierad `FIX LINK`. Inga nya P0, inga nya trasiga anchors, inga nya redirects, inga nya P2-poster och ingen ökning av P1. Tabellen beskriver checkpointen 2026-09-04; vid den tidpunkten var draft/noindex-logiken från Fas C orörd, sitemap oförändrad och `/referenser/`-urvalet 15 kort.

### 12.6 Nuläge vid återställning (2026-09-14)

Checkpointens tre verifierade `FIX LINK`-beslut återställdes mot aktuell `upstream/main` efter senare referens- och publiceringsändringar. Klassificeringarna under `BUILD PAGE` och `REVIEW` ändrades inte.

| Mått | Verifierat nuläge |
|---|---|
| P0-poster | **24** |
| P1-poster | **0** |
| P2-poster | **189** |
| Byggda HTML-sidor | 70 |
| Reella sidor | 56 |
| Indexerbara sidor | 42 |
| Noindex-sidor | 28 |
| Sitemap-routes | 42 |
| Sitemap-routes som är noindex | 0 |
| Orphan-sidor | 0 |
| Svagt internlänkade (≤2) | 0 |
| Referenskort i `/referenser/` | 16 |

De fem rättade länkförekomsterna är fortsatt giltiga mot dagens routes. Förändrade sid-, sitemap- och referensantal beror på senare mergade referens- och publiceringsändringar, inte på en omklassificering av Fas B-fynden.

### 12.7 Validering vid återställning (2026-09-14)

| Kommando | Resultat |
|---|---|
| `npm run build` | OK – Complete, 57 routes; 70 byggda HTML-sidor enligt audit |
| `node scripts/audit-internal-links.mjs` | OK – exit 0, P0 24, P1 0, P2 189 |
| `GITHUB_BASE_REF=upstream/main npm run validate` | OK – exit 0, "Guardrail validation passed for 16 reference content entries" |
| `git diff --check` | OK – exit 0, rent |
